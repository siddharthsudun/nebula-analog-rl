                ...
                for process in ("tt", "ss", "ff", "sf", "fs"):
                    for lane in range(lanes):
                        ...
                        proc = subprocess.Popen(
                            [sys.executable, "-u", "-m", "eqrl.pvt_workers", "--worker", process,
...
class ResidentEvaluator:
    ...
    def evaluate_batch_parallel(self, candidates, grid):
        from eqrl.pvt_refinement import EvaluationBudgetExceeded
...
def configuration_stamp():
    ...
    for path in paths:
        digest.update((ROOT/'src/eqrl'/path).read_bytes())
    for name in ('PDK_ROOT', 'NGSPICE_LIBRARY_PATH', 'SPICE_LIB_DIR', 'EQRL_PVT_LANES'):
...
def get_pool(deadline=None):
    ...
    with deadline_lock(_pool_lock, deadline):
        ...
        if _pool is None or _pool.closed:
            _pool = ResidentPool(lanes=int(os.environ.get("EQRL_PVT_LANES", "3")), deadline=deadline)
...
def worker_main(process, store):
    ...
    try:
        with redirect_stdout(sys.stderr):
            ...
            from eqrl.sim.server import get_server, NgspiceServer
            from eqrl.pvt_refinement import PVTEvaluator
