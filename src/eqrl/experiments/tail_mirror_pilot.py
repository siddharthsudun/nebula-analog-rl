...
Run: python -m eqrl.experiments.tail_mirror_pilot --output results/<new-directory>
...
FROZEN = (
    ...
    "src/eqrl/guards.py", "src/eqrl/sim/eye.py",
    "src/eqrl/experiments/final_report.py", "src/eqrl/specs.py",
    "src/eqrl/envs/equalizer_env.py", "src/eqrl/circuits/ctle.py",
)
CONFIGS = {
    "simple": TailConfig(),
...
def main():
    ...
    manifest = {"eval_limit": len(labels) * len(ratios) * 3, "wall_limit_seconds": wall_limit,
                ...
                "code_sha256": {
                    p: hashlib.sha256((ROOT / p).read_bytes()).hexdigest()
                    for p in ("src/eqrl/circuits/tail_variants.py", "src/eqrl/evaluator.py",
                              "src/eqrl/experiments/tail_mirror_pilot.py")},
    ...
    try:
        for label in labels:
            ...
            if remaining <= 0:
                break
            command = [sys.executable, "-m", "eqrl.experiments.tail_mirror_pilot",
