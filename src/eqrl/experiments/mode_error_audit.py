...
def child(job, destination):
    ...
    from eqrl.experiments import final_comparison  # toolchain setup, no main()
...
def dispatch(job, root, name):
    ...
    with (root / f"{name}.log").open("x", encoding="utf-8") as log:
        try:
            r = subprocess.run([sys.executable, "-m", "eqrl.experiments.mode_error_audit",
...
def run(root):
    ...
    inputs = [*MODELS, *(str(Path(m).with_name(Path(m).stem + "_train.json")) for m in MODELS),
              ...
              "src/eqrl/pipeline.py", "src/eqrl/experiments/fastest_hedge.py",
              "src/eqrl/experiments/final_comparison.py", "src/eqrl/sim/eye.py",
