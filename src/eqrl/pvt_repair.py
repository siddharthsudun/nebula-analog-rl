...
def search(output, evaluator=None):
    import cma
    import numpy as np
    from eqrl.circuits.ctle import DesignVars, encode_action, decode_action
    from eqrl.envs.pvt import corner_grid
    from eqrl.pvt_refinement import PVTEvaluator, summarize, full_grid_pass, rank_key, stress_corners
...
def verification(output):
    ...
    from eqrl.experiments.pvt_optimize import verify
    verify(output)
...
def run_repair(design, spec, *, output=None, seed=20260910, wall_seconds=WALL_SECONDS, notify=None):
    ...
    if os.environ.get('EQRL_PVT_BACKEND', 'resident') == 'resident':
    ...
    source_paths = ['src/eqrl/pvt_repair.py','src/eqrl/pvt_refinement.py','src/eqrl/pipeline.py',
                    'src/eqrl/experiments/pvt_optimize.py','src/eqrl/specs.py','src/eqrl/guards.py',
                    'src/eqrl/circuits/ctle.py','src/eqrl/sim/eye.py','results/delivered_circuit.json',
    ...
    for phase in ('search','verify'):
        ...
        with (output/f'{phase}.log').open('w',encoding='utf-8') as log:
            proc = subprocess.Popen([sys.executable,'-m','eqrl.pvt_repair','--worker',phase,'--output',str(output.resolve())],
...
def run_resident_repair(design, spec, *, output=None, seed=20260910,
    ...
    import queue
    from eqrl.pvt_workers import get_pool, ResidentEvaluator, WorkerFailure
    from eqrl.envs.pvt import corner_grid
    from eqrl.pvt_refinement import full_grid_pass
    from eqrl.experiments.pvt_optimize import verify
    output = Path(output) if output else ROOT/'results'/'pipeline_pvt'/uuid.uuid4().hex
    ...
    config = dict(spec=asdict(spec), seed=seed, wall_seconds=wall_seconds,
                  ...
                  initial=[dict(id='input', origin='pipeline PPO/G3.2 output', design=design),
                           dict(id='anchor', origin='fixed delivered sizing, remeasured for this specification', design=anchor)])
    write(output/'config.json', config)
    source_paths = ['src/eqrl/pvt_repair.py', 'src/eqrl/pvt_refinement.py', 'src/eqrl/pvt_workers.py',
                    'src/eqrl/pipeline.py', 'src/eqrl/experiments/pvt_optimize.py', 'src/eqrl/specs.py',
                    'src/eqrl/guards.py', 'src/eqrl/evaluator.py', 'src/eqrl/sim/measures.py',
                    'src/eqrl/sim/server.py', 'src/eqrl/sim/probe.py', 'src/eqrl/sim/eye.py',
                    'src/eqrl/circuits/ctle.py', 'results/delivered_circuit.json']
    write(output/'provenance.json', {p:hashlib.sha256((ROOT/p).read_bytes()).hexdigest() for p in source_paths})
    started = time.monotonic()
    result = dict(accepted=False, status='incomplete', artifact_dir=str(output.resolve()),
                  scope='schematic-level 45-corner PVT; no mismatch, extracted parasitics or SNR certification',
