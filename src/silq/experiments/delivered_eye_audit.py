...
def audit_one(dv, spec, proc, vdd, temp, version, store):
    ...
    from silq.guards import ArtifactStore, SearchHalted
...
def worker(proc, directory):
    ...
    from silq.experiments import final_comparison  # noqa: F401
...
def main():
    ...
    if a.worker:
        ...
        return
    plan = {"generator": "silq.experiments.delivered_eye_audit", "corners": 45,
    ...
    plan["inputs"] = [fingerprint(n) for n in
                      (SOURCE, "src/silq/sim/eye.py", "src/silq/sim/measures.py",
