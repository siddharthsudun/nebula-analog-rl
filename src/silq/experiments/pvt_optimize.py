...
from silq.circuits.ctle import DesignVars, decode_action, encode_action
from silq.envs.pvt import corner_grid
from silq.experiments.tail_mirror_pilot import ROOT, fingerprints, write_json
from silq.pvt_refinement import PVTEvaluator, full_grid_pass, rank_key, recorded_netlist, stress_corners, summarize
...
def main():
    ...
    manifest = {"frozen_before": before, "workers": [], "complete": False,
                "source_sha256": {p: hashlib.sha256((ROOT / p).read_bytes()).hexdigest()
                                  for p in ("src/silq/pvt_refinement.py", "src/silq/experiments/pvt_optimize.py",
    ...
    try:
        for phase in ("optimize", "verify"):
            ...
            with (args.output / f"{phase}.log").open("w", encoding="utf-8") as log:
                proc = subprocess.Popen([sys.executable, "-m", "silq.experiments.pvt_optimize",
