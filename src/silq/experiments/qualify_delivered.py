...
(silq.evaluator.build_evaluator), and writes results/qualification_delivered.json -- an
...
HONESTY, stated where the numbers are produced so a reader cannot miss it:
  ...
  * Part B here uses the fast (AC-path) evaluator for speed, so HD3 and input-referred
  ...
  them; do not shrink the stress after seeing the pass fraction.
    ...
    PYTHONPATH=src python -m silq.experiments.qualify_delivered            # 2000-sample MC
    PYTHONPATH=src python -m silq.experiments.qualify_delivered --mc-n 10000
"""
...
from silq.experiments.freeze_delivered import LIMITS
...
def build(mc_n: int, do_pairwise: bool, do_adversarial: bool,
    ...
    return {
        ...
        "generated_by": "silq.experiments.qualify_delivered",
        "commit": subprocess.run(["git", "rev-parse", "HEAD"], capture_output=True,
                                 text=True).stdout.strip(),
        "circuit_source": DELIVERED,
        "circuit_commit": delivered.get("commit"),
        "guard": "silq.evaluator.build_evaluator -- the same guard the CLI and dashboard use",
