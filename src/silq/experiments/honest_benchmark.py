...
`measure_all` call and never consults the guard layer (silq.guards). The RL policy,
...
def main() -> None:
    ...
    p.add_argument("--require-valid", action="store_true",
                   help="a solve must ALSO be guard-valid (silq.guards Tiers 1-4), "
    ...
    if not model_path.exists():
        raise SystemExit(
            ...
            f"  python -m silq.agents.train_sequential --guarded --no-fast "
            f"--out {model_path}")
