...
on the site, the manifest is right.
    ...
    PYTHONPATH=src python -m silq.experiments.freeze_delivered
    PYTHONPATH=src python -m silq.experiments.freeze_delivered --verify   # exit 1 on drift
"""
...
from silq.sim.server import AC_DECADE_PTS, AC_FSTART, AC_FSTOP
...
def build() -> dict:
    from silq.experiments.pvt_signoff import flagship, score
    ...
    return {
        ...
        "generated_by": "silq.experiments.freeze_delivered (measures nothing)",
        ...
        "measurement": {
            "instrument": "silq.evaluator.build_evaluator(fast=False) — full guard layer, "
                          "real HD3 and noise",
