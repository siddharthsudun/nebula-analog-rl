...
`silq.pipeline.design` no longer calls them for live `mode="fastest"` runs.
"""
...
from silq.experiments.final_comparison import DIMS, PREREG

#: Mirrors `silq.surrogate._CORPUS_DEFAULT`. Not imported directly because that name is
...
def propose_seed(target: float, surrogate, spec=None) -> np.ndarray:
    ...
    `silq.surrogate`'s own module docstring is explicit that `boost_db`, `dc_gain_db` and
...
def surrogate_stage1(evaluate, target: float, surrogate, spec=None, *, retry_on_guard_invalid: bool = True,
    ...
    corpus proposes, it never gets to decide (`silq.surrogate`: "IT IS A RANKER, NEVER A
