...
runs. `silq.experiments.simcount_audit` established the conversion factor by patching those
...
could use it, and `silq.pipeline` ended up reporting a hand-derived figure instead.
...
WHAT GETS PATCHED, AND WHY THREE NAMES FOR TWO CHOKEPOINTS. `silq.evaluator` reaches
measure_all through the module object (`_m.measure_all`), so patching
`silq.sim.measures.measure_all` catches it. `silq.envs.sequential_env` did
`from silq.sim.measures import measure_all` at import time and therefore holds its own
...
MEASURE_ALL_BINDINGS = (
    ("silq.sim.measures", "measure_all"),      # what silq.evaluator resolves through
    ("silq.envs.sequential_env", "measure_all"),   # imported by name at module import
)
...
def counting() -> Iterator[dict]:
    ...
    base = originals[("silq.sim.measures", "measure_all")]
