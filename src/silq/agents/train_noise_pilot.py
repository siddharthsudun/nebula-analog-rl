...
def _source_hashes(repo_root: Path) -> dict[str, str]:
    ...
    source_root = repo_root / "src" / "silq"
    hashes: dict[str, str] = {}
...
def _provenance(repo_root: Path) -> dict[str, Any]:
    return {
        ...
        "module": "silq.agents.train_noise_pilot",
    }
...
def _make_callback(run_dir: Path, config: dict[str, Any], deadline: float) -> Any:
    ...
    class PilotCallback(BaseCallback):
        ...
        def _payload(self, status: str = "running") -> dict[str, Any]:
            ...
            return {
                "schema": "silq.noise_pilot.progress.v2",
                "status": status,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        from silq.experiments.noise_holdout import cases, evaluate_policy, regressed
        import torch
        manifest = cases()
        ...
        if config['profile'] == 'full' and status == 'completed':
            from silq.experiments.noise_holdout import qualification_cases
        ...
        final = {
            "schema": "silq.noise_pilot.status.v2",
            "status": status,
