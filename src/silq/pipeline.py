...
`experiments/final_comparison.py`, but the public entry point `silq.solve` ran PPO alone
...
delivered circuit. This module is the one place that architecture lives, and `silq.solve`
...
result is real and not a cached number.
    ...
    from silq.pipeline import design
...
deliberately deferred to call time so that `import silq.pipeline` stays cheap and works on a
...
#: which replaces stage 1 with a corpus lookup; see `silq.experiments.fastest_hedge`.
...
#: Fastest: stage 1 is a corpus lookup (silq.experiments.fastest_hedge.surrogate_stage1) --
...
#: THINKING_SURROGATE_STARTS corpus-proposed starts (`silq.experiments.thinking_starts`).
...
#: (`silq.experiments.axis_retarget`). Budget and stopping tolerance are deliberately left
...
def _reselect_for_requirements(result: dict, trace: list, spec: Spec, tol: float,
    ...
    if not set(v.get("failing") or []) <= user_checks:
        return
    from silq.request_budget import active
...
def _design_nominal(target_boost_db: float, channel_loss_db: float = DEFAULT_SPEC.channel_loss_db,
    ...
    lookup plus a single real evaluation (`silq.experiments.fastest_hedge.surrogate_stage1`)
    ...
    if mode in ("thinking", "g32_acceptance"):
        ...
        # See silq.experiments.thinking_starts: the failures this addresses are ones where
        ...
        from silq.guards import DC_GAIN_DB_MIN
        ...
        def _headroom(c):
            ...
            fitted quantity (`silq.experiments.g32_headroom`): DC_GAIN_DB_MIN is 0.0 dB.
            """
        ...
        # See silq.experiments.fastest_hedge.surrogate_stage1's docstring for why this is
        ...
        from silq.experiments.fastest_hedge import load_fastest_assets, surrogate_stage1
        ...
        if mode == "retarget":
            from silq.experiments.axis_retarget import (RETARGET_SURROGATE_OPTIONAL,
                                                        retarget_stage2)
    ...
    result: dict[str, Any] = {
        ...
        "spec": {"target_boost_db": target_boost_db, "channel_loss_db": channel_loss_db,
                 ...
                 "requirement_set": ("silq.specs.Spec defaults + this target/channel; "
    ...
    if allow_fallback:
        ...
        result["provenance"].update({
            ...
            "fallback_source": "silq.baselines.robust.robust_design",
...
def design(target_boost_db: float, channel_loss_db: float = DEFAULT_SPEC.channel_loss_db,
    ...
    if wall_seconds is not None:
        from silq.realtime import design_realtime
