...
#: the demo wants a fast answer. Override with SILQ_SPEC_MODEL. Kept in one place so a
#: model retirement is a one-line fix rather than a grep.
DEFAULT_MODEL = os.getenv("SILQ_SPEC_MODEL", "claude-sonnet-5")
...
CLI_TIMEOUT_S = float(os.getenv("SILQ_SPEC_CLI_TIMEOUT", "25"))
...
def _cli_path() -> str | None:
    ...
    if os.getenv("SILQ_SPEC_LLM", "").lower() == "off":
        return None
    explicit = os.getenv("SILQ_CLAUDE_CLI")
...
def _llm_cli(text: str, model: str | None) -> str:
    ...
    m = model or os.getenv("SILQ_SPEC_CLI_MODEL", "sonnet")
    if m:
        args += ["--model", m]
...
def _pick_backend(backend: str) -> str | None:
    ...
    env_pref = os.getenv("SILQ_SPEC_LLM", "auto").lower()
    if env_pref == "off":
        return None
    if env_pref == "cli":
        return "cli" if _cli_path() else None
...
def parse_spec_verbose(text: str, model: str | None = None, *,
    ...
    from silq.llm.snr_parser import parse_noise_intent
