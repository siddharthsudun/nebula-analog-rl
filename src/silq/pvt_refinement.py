...
from silq.specs import Spec, hard_pass
...
"""
    from silq.guards import SATURATION_HEADROOM_V, TAIL_CURRENT_TOLERANCE
...
class PVTEvaluator:
    ...
    def evaluate_batch(self, candidates: list[dict], grid: list[tuple]) -> dict[str, list[dict]]:
        ...
        from silq.sim.probe import CTLE_NODES, ProbeError, probe_operating_point
    ...
    def evaluate_batch_parallel(self, candidates: list[dict], grid: list[tuple]) -> dict[str, list[dict]]:
        ...
        with tempfile.TemporaryDirectory(prefix="pvt_shard_") as tmp:
            tmp = Path(tmp)
            running = []
            from silq.pvt_workers import WindowsJob
            ...
            try:
                for i, (proc, shard_grid) in enumerate(shards.items()):
                    ...
                    p = subprocess.Popen(
                        [sys.executable, "-m", "silq.pvt_refinement", str(payload)],
                ...
                for proc, p, log, payload in running:
                    ...
                    from silq.simcount import add_worker_counts
