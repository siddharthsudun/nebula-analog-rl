...
from silq.pvt_workers import WindowsJob, ROOT, configuration_stamp
from silq.realtime import LIMITS, empty_result
...
def stamp():
    ...
    for name in ('pipeline.py','runtime.py','realtime.py','request_budget.py','pareto.py','pvt_fast.py','pvt_repair.py'):
        digest.update((ROOT/'src/silq'/name).read_bytes())
...
class Runtime:
    def __init__(self):
        ...
        env.setdefault('SILQ_PVT_LANES','3')
        env['PYTHONUTF8']='1'
        for name in ('OMP_NUM_THREADS','MKL_NUM_THREADS','OPENBLAS_NUM_THREADS','NUMEXPR_NUM_THREADS'):env[name]='1'
        self.ready=queue.Queue()
        self.proc=subprocess.Popen([sys.executable,'-u','-m','silq.runtime','--worker'],cwd=ROOT,env=env,
